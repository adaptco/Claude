...
class ByteSamplerAdapter:
    ...
    def sample_next_bytes(self,
        ...
        Args:
            ...
            constraints: Optional constraints on sampling
    ...
    def get_distribution_from_paths(self,
        ...
        if total_prob > 0:
            byte_probs = {b: p / total_prob for b, p in byte_probs.items()}
...
if __name__ == "__main__":
    ...
    print(f"  Total paths: {dist.total_paths}")
    print(f"  Continuations:")
    for byte_seq, prob in sorted(dist.distributions.items(), key=lambda x: -x[1])[:5]:
        print(f"    {byte_seq[:20]}: {prob:.4f}")
    
    # Sample next bytes
    sampled = adapter.sample_next_bytes(prefix, max_length=10)
    print(f"\nSampled: {sampled}")
    ...
    print(f"Valid token paths: {len(paths)}")
